...
Game::Game(sf::RenderWindow &window, const Level &lvl):
    window(window),
    info(nullptr),
...
void Game::frameStep(float ms_time){
    auto points = cmd_list->apply(logger);
    red_points += points.first;
    blu_points += points.second;
...
void Game::addPoints(Team team, int score){
...
void Game::save(const std::string &name) const
{
    auto lvl = field->save();
    lvl.saveToFile(name);
}
...
void Game::mousePressEvent(int button, int x, int y){
    press_x = x;
    press_y = y;
...
void Game::mouseReleaseEvent(int button, int x, int y){
    old_x = x;
    old_y = y;
...
void Game::mouseMoveEvent(int new_x, int new_y){
    int dx = new_x - old_x;
    int dy = new_y - old_y;
...
void Game::keyPressedEvent(int button){
...
void Game::resizeEvent(unsigned int uNewWidth, unsigned int uNewHeight){
    auto view = window.getView();
...
void Game::handlePressInterface(const std::string &button){
    draw_ways = 0;
    draw_targets = 0;
...
void Game::handleReleaseInterface(const std::string &button){
    if(button == "move"){
        draw_ways = 0;
    }
...
void Game::handlePressCell(int mouse_button,
                           const std::vector<std::string> &interface_buttons,
                           int x, int y)
{
        ...
        for(auto b: interface_buttons){
            ...
            if(b == "build_tower"){
                ...
                if(abs(offset.x) + abs(offset.y) == 1){
                    ...
                    catch(CellInterface::empty_cell e){
                        std::cout << e.what() << std::endl;
                    }
                }
            }
