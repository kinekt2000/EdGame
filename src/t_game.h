...
public:
    ...
    void frameStep(float ms_time)
    {
        ...
        float real_red_points = float(points.first) * points_mod;
        if(real_red_points > 0) real_red_points = ceil(real_red_points);
        else real_red_points = floor(real_red_points);

        float real_blu_points = float(points.second) * points_mod;
        if(real_blu_points > 0) real_blu_points = ceil(real_blu_points);
        else real_blu_points = floor(real_blu_points);

        red_points += int(real_red_points);
        blu_points += int(real_blu_points);
